function [t, y] = ode_ralston(dydt, tspan, y0, h)
%ODE_RALSTON  Ralston's method (2nd-order Runge-Kutta) for dy/dt = f(t,y).
%   [t, y] = ode_ralston(dydt, tspan, y0, h)
...
%   Formula: k1=f(t,y); k2=f(t+3h/4, y+3*k1*h/4); y_new = y+(k1/3+2*k2/3)*h
%   Example: [t,y] = ode_ralston(@(t,y) y.*t.^3-1.5*y, [0 2], 1, 0.5)
...
for i = 1:n-1
    ...
    k2 = dydt(t(i) + 0.75*hi, yy + 0.75*k1*hi); k2 = k2(:);
    Y(i+1,:) = (yy + (k1/3 + 2*k2/3)*hi)';
end
y = Y;
if nargout == 0
    ru_print_ode_table('ode_ralston', t, Y, m);
end
end
